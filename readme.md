...
1. add a folder inside `plugins` named after the plugin name
2. add a `plugin.py` file that will be executed
3. expect the mail being passed in stdin as a json string with the following properties
...
4. [optional] if dependencies are needed, add a `requirements.txt` file
5. [optional] environment variables for the script can be included in a `.env` file
...
Some examples are present in `builtins` folder, which is intended to be shipped with the docker image although not strictly required to run the application.
